    ...
    public partial class Form1 : Form
    {
        public Form1()
        {
			...
			try
			{
				...
				{
					treeNode_procedure.Nodes.Add("", ds1.Tables[0].Rows[i][0].ToString(), 0, 0);
				...
				{
					treeNode_sequence.Nodes.Add("", ds2.Tables[0].Rows[i][0].ToString(), 0, 0);
				}
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
			...
			else
			{
				...
				try
				{
					...
					if (e.Node.Parent.Text == "테이블")
					{
						textBox1.Text = null;
						OracleCommand pgOraCmd = new OracleCommand("select * from " + e.Node.Text, pgOraConn);
						OracleDataAdapter oda = new OracleDataAdapter(pgOraCmd);
						DataSet ds_temp = new DataSet();
						oda.Fill(ds_temp);
						ds = ds_temp;
					}
					if (e.Node.Parent.Text == "프로시저")
					{
						OracleCommand pgOraCmd = new OracleCommand("SELECT * FROM user_source WHERE name=upper('" + e.Node.Text + "')", pgOraConn);
						OracleDataAdapter oda = new OracleDataAdapter(pgOraCmd);
						DataSet ds_temp = new DataSet();
						oda.Fill(ds_temp);
						ds = ds_temp;
					...
					if (e.Node.Parent.Text == "시퀀스")
					{
						textBox1.Text = null;
						OracleCommand pgOraCmd = new OracleCommand("SELECT * FROM USER_SEQUENCES WHERE SEQUENCE_NAME = '" + e.Node.Text + "'", pgOraConn);
						OracleDataAdapter oda = new OracleDataAdapter(pgOraCmd);
						DataSet ds_temp = new DataSet();
						oda.Fill(ds_temp);
						ds = ds_temp;
